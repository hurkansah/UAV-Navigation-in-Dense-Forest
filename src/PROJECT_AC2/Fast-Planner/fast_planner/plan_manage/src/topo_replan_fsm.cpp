...
void TopoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  fp_state = nh.advertise<std_msgs::String>("/planning/fp_state", 1000);
  emergency_state = nh.advertise<std_msgs::String>("/planning/fp_emergency_state", 1000);
}
...
void TopoReplanFSM::printFSMExecState() {
  ...
  str.data = state_str[int(exec_state_)];
  fp_state.publish(str);
  std_msgs::String str_emr;
  str_emr.data = "MOVE";
  emergency_state.publish(str_emr);
}
  ...
  if (exec_state_ == EXEC_TRAJ || exec_state_ == REPLAN_TRAJ) {
    ...
    if (!safe) {
      if (dist > 0.4) {
        ...
        str_emer.data = "STOP";
        emergency_state.publish(str_emer);
