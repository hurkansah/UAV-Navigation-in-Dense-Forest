...
void publish_topics(ros::Time msg_time,Eigen::Vector3f Wbb)
{
    ...
    //Sophus::SE3f Twmp = Twc;
    std::vector<Sophus::SE3f> transformedPoses;
    std::vector<Sophus::SE3f> mavros_pose; // Try to publish mavros pose ----
    std::vector<Sophus::SE3f> allPoses = pSLAM->GetAllKeyframePoses();
   ...
   //if (sensor_type == ORB_SLAM3::System::MONOCULAR || sensor_type == ORB_SLAM3::System::STEREO)
    //{
        //Sophus::SE3f Twc = pSLAM->GetCamTwc();
        Eigen::AngleAxisf AngleR(1.57079632679, Eigen::Vector3f::UnitX()); // 1.57079632679
        Eigen::AngleAxisf AngleP(0, Eigen::Vector3f::UnitY());
        Eigen::AngleAxisf AngleY(1.57079632679, Eigen::Vector3f::UnitZ()); // 1.57079632679

        Eigen::Quaternionf qRPY = AngleR * AngleP * AngleY;
        Eigen::Matrix3f RotRPY = qRPY.matrix();
        Tc0w = Sophus::SE3f(RotRPY, Eigen::Vector3f::Zero());

        Sophus::SE3f Twmp =  Tc0w.inverse() * Twc;//Tc0w.inverse() * Twc; // new frame 

        //std::vector<Sophus::SE3f> allPoses = pSLAM->GetAllKeyframePoses();
        //std::vector<Sophus::SE3f> transformedPoses;
        // get the pose information according to the new frame 
        for(const auto& pose : allPoses) {
            Sophus::SE3f transformedPose = Twmp * pose;
    ...
    if (sensor_type == ORB_SLAM3::System::IMU_MONOCULAR || sensor_type == ORB_SLAM3::System::IMU_STEREO || sensor_type == ORB_SLAM3::System::IMU_RGBD)
    {
        ...
        Twb = Tc0w.inverse()*Twb;
        Eigen::Vector3f Vwb = pSLAM->GetImuVwb();
        Vwb = Tc0w.inverse()*Vwb;
        ...
        //Sophus::SE3f Twmp = pSLAM->GetCamTwc();
        //transformedPoses =  pSLAM->GetAllKeyframePoses();
    ...
    publish_camera_pose(Twmp, msg_time);

    publish_tf_transform(Twmp, world_frame_id, cam_frame_id, msg_time);
    ...
    publish_kf_markers(transformedPoses, msg_time);
}
...
sensor_msgs::PointCloud2 mappoint_to_pointcloud(std::vector<ORB_SLAM3::MapPoint*> map_points, ros::Time msg_time)
{
    ...
    for (unsigned int i = 0; i < cloud.width; i++)
    {
        if (map_points[i])
        {
            ...
            double f_scale = 1; //0.9;

            // Map point frame transformation, X front Y side and Z up 
            ...
            if (sensor_type == ORB_SLAM3::System::MONOCULAR || sensor_type == ORB_SLAM3::System::STEREO)
             {
                ...
                Eigen::Matrix3f RotRPY = qRPY.matrix();
                Tc0w = Sophus::SE3f(RotRPY, Eigen::Vector3f::Zero());
                double scale_alpha = 2; // point cloud scale adjustment according to environment
                //Sophus::SE3f Tc0mp(Eigen::Matrix3f::Identity(), P3Dw);
                //Sophus::SE3d Tc0mp(Eigen::Matrix3d::Identity(), P3Dw.cast<double>());
                Sophus::SE3f Tc0mp(Eigen::Matrix3f::Identity(), P3Dw.cast<float>());
                Sophus::SE3f Twmp = (Tc0w.inverse() * Tc0mp).cast<float>();
                P3Dw = (Twmp.cast<double>()).translation();
            ...
            float data_array[num_channels] = {
                f_scale*point_translation.x(),
                f_scale*point_translation.y(),
                f_scale*point_translation.z()
            };
